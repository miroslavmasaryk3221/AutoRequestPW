...
$dest="https://chromedriver.storage.googleapis.com/LATEST_RELEASE_$ChromeVersion"
$proxyurl = ([System.Net.WebRequest]::GetSystemWebproxy()).GetProxy($dest)
$ChromeDriverVersion = (Invoke-WebRequest "https://chromedriver.storage.googleapis.com/LATEST_RELEASE_$ChromeVersion"-Proxy $proxyurl -ProxyUseDefaultCredentials).Content ;
...
If ($IsWindows -or $Env:OS) {
    $dest="https://chromedriver.storage.googleapis.com/LATEST_RELEASE_$ChromeVersion"
    $proxyurl = ([System.Net.WebRequest]::GetSystemWebproxy()).GetProxy($dest)
    Invoke-WebRequest "https://chromedriver.storage.googleapis.com/$ChromeDriverVersion/chromedriver_win32.zip" -OutFile $TempZipFilePath -Proxy $proxyurl -ProxyUseDefaultCredentials;
