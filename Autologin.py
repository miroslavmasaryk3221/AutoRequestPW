...
def main():
    ...
    print("[Changelog]")
    print(
        "[v1.0 11/05/2021 First working version, current problem: while entering any string auto (enter ?) is present ]")
    print(
        "[v1.1 12/05/2021 Fixed previous error, problem was while reading file,removed unvisible chars from strings, some elements has still problems]")
    print("[v1.2 12/05/2021 All elements can be found now.")
    print(
        "[v1.3 17/05/2021 Fixed unable to locate element while copying password. Explicit wait time raised up to 90 seconds]")
    ...
    print(
        "[v1.6 31.08/2021  Option to choose chromedriver removed, chromedriver will be automatically downloaded upon current exisitng chrome version")
    time.sleep(3)
    ...
    WebDriverWait(driver, 90).until(
        expected_conditions.presence_of_element_located((By.ID, "edit-secret-password-copy"))).click()
    time.sleep(2)
    ...
    time.sleep(2)
    WebDriverWait(driver, 90).until(
        expected_conditions.presence_of_element_located((By.XPATH, "//img[@alt=\'RDP Client\']"))).click()
